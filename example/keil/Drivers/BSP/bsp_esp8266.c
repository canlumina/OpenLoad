...
#include "stm32f1xx.h"	 //包含需要的头文件
#include "bsp_esp8266.h" //包含需要的头文件
...
void WiFi_ResetIO_Init(void)
{
	...
	GPIO_InitStructure.Pin = GPIO_PIN_4;				// 准备设置PA4
	GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_HIGH;	// 速率50Mhz
	GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_PP; // 推免输出方式
	HAL_GPIO_Init(GPIOA, &GPIO_InitStructure);				// 设置PA4
	RESET_IO(1);										// 复位IO拉高电平
}
/*-------------------------------------------------*/
/*函数名：WiFi发送设置指令                          */
