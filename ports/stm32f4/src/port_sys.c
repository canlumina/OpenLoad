...
/* Cortex-M4F 跳转: 取 [addr] 为 MSP, [addr+4] 为 Reset_Handler 入口.
 *
 * F4 跟 F1 多一步: 关 FPU. bootloader 启用了 FPU (CPACR.CP10/11=11),
 * 而 app 没启 FPU 时, app 首次浮点指令会硬错. 跳前清 CPACR 安全兜底. */
static void sys_jump(uint32_t app_addr)
{
    /* 0. 关 FPU access (FPU 由 SystemInit 启用, 跳之前清回 reset 状态) */
